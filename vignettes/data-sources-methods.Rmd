...
- **NA propagation in `exp_cur_total`.** Missing F-33 items now propagate to
  `NA` rather than being treated as zero, so current-expenditure totals differ
  from 0.1.x for districts with incomplete reporting. This is the correct
  behavior (an unreported item is unknown, not zero), but it changes published
  numbers for affected district-years.
